...
class EarlyOut(Callback):
    """Stop training when a monitored quantity crosses a baseline.
    ...
    def __init__(self,
        ...
        if mode not in ['auto', 'min', 'max']:
            warnings.warn('EarlyOut mode %s is unknown, '
                          'fallback to auto mode.' % mode,
                          RuntimeWarning)
            mode = 'auto'
    ...
    def get_monitor_value(self, logs):
        ...
        if monitor_value is None:
            warnings.warn(
                'Early Out conditioned on metric `%s` '
...
class EarlyStopping(Callback):
    ...
    # Arguments
        ...
        mode: one of {auto, min, max}. In `min` mode,
            ...
            from the name of the monitored quantity.
        baseline: Baseline value for the monitored quantity to reach.
            Training will stop when the model reaches this baseline.
        restore_best_weights: whether to restore model weights from
            the epoch with the best value of the monitored quantity.
            If False, the model weights obtained at the last step of
            training are used.
